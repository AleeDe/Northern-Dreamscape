...
export const landmarkFields = groq`
  _id,
  name,
  "slug": slug.current,
  category,
...
export const packageBySlugQuery = groq`
  *[_type == "package" && slug.current == $slug && status == "live"][0] {
    ...
    destination->{
      ...
      tagline, summary, bestSeason, elevationRange, climate, travelTime,
      heroImage { ${imageFields} },
      "landmarks": *[_type == "landmark" && references(^._id) && status == "live"] { ${landmarkFields} },
      accommodations[]->{ ${serviceCardFields}, shortDescription, highlights },
      vehicles[]->{ ${serviceCardFields}, shortDescription, seats, withDriver },
      guides[]->{ ${serviceCardFields}, shortDescription, languages, experienceYears, portrait { ${imageFields} } },
      restaurants[]->{ ${serviceCardFields}, shortDescription, cuisines, priceRange }
    },
    itineraryDays[] {
      ...,
